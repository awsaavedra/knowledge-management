#!/usr/bin/env bats
# Tests for public/inbox/templates/ — the canonical markdown templates for every
...
setup() {
    common_setup
    TEMPLATES_DIR="${PROJECT_ROOT}/public/inbox/templates"
}
...
@test "public/inbox/templates/ directory exists" {
...
@test "every template names a Producer in its spec block" {
    for tpl in "${EXPECTED_TEMPLATES[@]}"; do
        head -10 "${PROJECT_ROOT}/public/inbox/templates/${tpl}" | grep -q 'Producer:'
    done
}
...
@test "every template has YAML frontmatter after the spec block" {
    for tpl in "${EXPECTED_TEMPLATES[@]}"; do
        ...
        local count
        count=$(head -30 "${PROJECT_ROOT}/public/inbox/templates/${tpl}" | grep -c '^---$' || true)
        [ "$count" -ge 2 ]
    done
}
...
@test "no leftover *-format-template* templates at top of inbox" {
    ...
    # only exist under public/inbox/templates/.
    ! ls "${PROJECT_ROOT}/public/inbox/"*-format-template.md 2>/dev/null
}
