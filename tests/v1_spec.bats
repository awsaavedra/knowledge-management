...
# =============================================================================
# v0 hard-fails (exits non-zero or silently skips) when a note's frontmatter
# uses YAML block sequence format instead of inline array.
#
# Broken input:
#   tags:
#     - foo
#     - bar
#
# Expected: okm tags/tag/untag/tagged all parse this as equivalent to [foo, bar].
# Fix: in bin/okm, the frontmatter tag-extraction awk/sed must handle both
...
# =============================================================================
# has_frontmatter() returns true for any file whose first line is "---",
# including notes where "---" is a standalone horizontal rule (no closing ---).
# Consequence: write_tags_line is called on such notes, finds no tags: line,
# and silently no-ops — no tag written, no error emitted.
#
# Failing case: note with ONLY a leading --- (no closing ---)
# Fix: has_frontmatter must confirm BOTH an opening --- at line 1 AND a closing
# --- on a later line with valid YAML key:value content between them.
...
# =============================================================================
# N31: write_tags_line clobbers file permissions (644 → 600)
# =============================================================================
# write_tags_line writes via: tmp=$(mktemp); ... > $tmp; mv $tmp $original
# mktemp creates files with 600 (owner-only). mv replaces the original,
# inheriting 600 regardless of the original's permissions.
#
# Fix (pick one):
#   Option A: capture perms before write, chmod after mv.
#     orig_perms=$(stat -c %a "$file"); mv "$tmp" "$file"; chmod "$orig_perms" "$file"
#   Option B: write to tmp in same directory as original (same filesystem),
